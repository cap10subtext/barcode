...
class Code128 extends BarcodeBase
{
	 ...
	 */
	protected $subtype = self::TYPE_AUTO;
	 ...
	 * Set the label
	 *
	 * @param string - (string) string to add as label
	 * @return instance of \emberlabs\Barcode\BarcodeInterface
	 * @return throws \OverflowException
	 */
	...
	{
		$this->subtype = ($subtype < 1 || $subtype > 3) ? self::TYPE_AUTO : (int) $subtype;
	}
	...
	public function draw()
	{
		...
		$numBarsRequired = ($this->subtype != self::TYPE_C) ? (sizeof($charAry) * 11) + 35 : ((sizeof($charAry)/2) * 11) + 35;
		...
		foreach($charAry as $k => $char)
		{
			...
			{
				imagefilledrectangle($this->img, $currentX, $yoffset , ($currentX + $pxPerBar), ($this->y - 1), (($bit == '1') ? $black : $white));
				$currentX += $pxPerBar;
			}
		}
		...
		if($this->humanText){
		    ...
		    ImageString ($this->img, 4,0, 0, $label, $black);
		}
